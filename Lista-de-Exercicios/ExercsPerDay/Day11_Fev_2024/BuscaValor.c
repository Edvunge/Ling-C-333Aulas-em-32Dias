/*
1. (buscaValor) Escreva um programa que leia uma matriz de inteiros 10 × 10 e um valor X. 
O programa deverá fazer uma busca na matriz por este valor e informar a localização (linha x coluna)
quando encontrar ou a mensagem de Valor não encontrado, caso contrário.
*/
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
...
int main(void) {
    ...
    // imprimir na tela os valores da matriz    
    for (int i = 0; i < LINHAS; i++) {  
        for (int j = 0; j < COLUNAS; j++) {     
            printf(" %d ", mat[i][j]);
            matAUX[i][j] = mat[i][j];
